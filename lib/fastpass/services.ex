defmodule Fastpass.Services do
  ...
  def add_service(user_id, attrs \\ %{}) do
    case Repo.get_by(Service, [service_letter: attrs.service_letter, branch_id: attrs.branch_id]) do
      nil ->
      ...
      _ -> {:error, "This service letter already exist"}
    end
  end
  ...
  def is_working(service_id) do
    ...
    query = from s in Service,
                 ...
                 on: s.working_time_group_id == wtg.id,
      ...
      fn ->
        results =
          Enum.map(
            attrs,
            fn desk_service ->
              case Branches.is_user_owner(user_id, desk_service.desk_id, :desk) and Branches.is_user_owner(
                user_id,
                desk_service.service_id,
                :service
                   ) do
                true ->
                  %DeskService{}
